...
_WORD_NUM = {"один": 1, "одна": 1, "одну": 1, "два": 2, "две": 2, "три": 3, "четыре": 4, "пять": 5, "шесть": 6,
             ...
             "пятьсот": 500}
_MULT = [(r"млн|миллион\w*", 1_000_000), (r"к|k|тыс\w*|тыщ\w*|тысяч\w*|тысяча|тысячу|тысячи", 1000)]
...
def parse_amount(text: Optional[str]) -> Optional[float]:
    ...
    for m in re.finditer(r"(?<![\d:.])(\d[\d\s]*(?:[.,]\d+)?)(?:\s*(млн|миллион\w*|тысяч\w*|тыс\w*|тыщ\w*|к|k)\b)?"
                         r"(?![\d:])", t):
